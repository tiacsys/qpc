    ...
    uint_fast8_t const qs_id)
{
    ...
    Q_ASSERT_NOCRIT_(210, r == Q_RET_TRAN);
    ...
    do {
        ...
        while (me->temp.fun != t) {
            ++ip;
            QF_CRIT_E_();
            Q_ASSERT_NOCRIT_(220, ip < QHSM_MAX_NEST_DEPTH_);
            QF_CRIT_X_();
            path[ip] = me->temp.fun;
            (void)QHSM_RESERVED_EVT_(me->temp.fun, Q_EMPTY_SIG);
        }
    ...
    uint_fast8_t const qs_id)
{
    ...
    QF_CRIT_E_();
    Q_REQUIRE_NOCRIT_(302, (QEvt_verify_(e))
                      && (s != Q_STATE_CAST(0))
    ...
    if (r >= Q_RET_TRAN) {
        ...
        while (QHSM_RESERVED_EVT_(t, Q_INIT_SIG) == Q_RET_TRAN) {
            ...
            QF_CRIT_E_();
            Q_ASSERT_NOCRIT_(410, ip < QHSM_MAX_NEST_DEPTH_);
            QF_CRIT_X_();
    ...
    QStateHandler const state)
{
    QF_CRIT_STAT_
    QF_CRIT_E_();
    Q_REQUIRE_NOCRIT_(602, me->state.uint == (uintptr_t)(~me->temp.uint));
    QF_CRIT_X_();
                ...
                if (me->temp.fun == path[0]) {
                    ...
                    while (r == Q_RET_SUPER) {
                        ...
                        if (me->temp.fun == s) { // is it the source?
                            ...
                            // entry path must not overflow
                            QF_CRIT_E_();
                            Q_ASSERT_NOCRIT_(510, ip < QHSM_MAX_NEST_DEPTH_);
                            QF_CRIT_X_();
                    ...
                    if (iq == 0) {
                        // entry path must not overflow
                        QF_CRIT_E_();
                        Q_ASSERT_NOCRIT_(520, ip < QHSM_MAX_NEST_DEPTH_);
                        QF_CRIT_X_();
